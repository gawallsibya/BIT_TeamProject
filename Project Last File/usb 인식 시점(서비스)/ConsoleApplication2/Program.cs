...
namespace ConsoleApplication2
{
    class Program
    {
        ...
        {
            //Timer t = new Timer();
            //t.Elapsed += new ElapsedEventHandler(OnTimedEvent);//시간 간격이 발생하면 호출한다.
            // t.Interval = 1000;
            // t.Start();
            // Console.ReadLine();
            //OnTimedEvent();

            //T = new System.Threading.Thread(usbinfo);
            //T.Start();
            //Console.ReadKey();

            DeviceChangeNotifier.Start();
            //Process.Start(@"C:\Users\77\Desktop\바탕화면\0617 바탕화면(후킹성공)\WpfApplication1\bin\Debug\WpfApplication1.exe", "000000000000001220" );
            Console.ReadKey();
        }
    }
    ...
    public class DeviceChangeNotifier : Form
    {
                ...
                foreach (DriveInfo d in allDrives)
                {
                    if (d.IsReady == true)
                    {
                        if (d.DriveType.ToString() == "Removable")
                        {
                            ...
                            //File.AppendAllText(path, serial);
                            //xml.Xml_Find(serial);
                            mRun(serial);
                        }
                    }
        ...
        public void Run(string serial)
        {
            ...
            string command = @"C:\Users\m\Desktop\프로젝트\프로젝트 0821\UserWallPaper\WpfApplication1\bin\Debug\UserWallPaper.exe";
            command += " " + serial;
