    ...
    public class FluidMouseDragBehavior : Behavior<UIElement>
    {
        ...
        void OnPreviewMouseDown(object sender, MouseButtonEventArgs e)
        {
            if (sender is UserControl)
                pt = e.GetPosition((UserControl)sender);
        ...
        void OnPreviewMouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (sender is UserControl && pt == e.GetPosition((UserControl)sender))
            {
                flag = 1;
                return;
            }
        ...
        void OnPreviewMouseUp(object sender, MouseButtonEventArgs e)
        {
            if (e.ChangedButton == DragButton)
            {
                ...
                if ((fElem != null) && (parentFWPanel != null))
                {
                    ...
                    System.Windows.Application.Current.Properties["DragAndDrop"] = true;
                }
                if (flag == 1)
                {
                    flag = 0;

                    if (sender is UserFile)
                    {
                        Process ps = new Process();
                        ps.StartInfo.FileName = ((UserFile)sender).path;
                        ps.Start();
                    }
                    else if (sender is UserFolder)
                    {
                        Process ps = new Process();
                        ps.StartInfo.FileName = ((UserFolder)sender).path;
                        ps.Start();
                    }
                }
            }
